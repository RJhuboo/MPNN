...
class Datasets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image,mask=Image.to_pil_image(torch.tensor(image)),TF.to_pil_image(torch.tensor(mask))
...
def test(model,testloader,epoch,opt):
    ...
    # Loading Checkpoint if we use this function as a testing function, Not necessary for hyperparameter tuning.
    if opt['mode'] == "Test":
...
def objective(trial):
    ...
    while True:
        i += 1
        if os.path.isdir("./result/cross_7p_human"+str(i)) == False:
            save_folder = "./result/cross_7p_human"+str(i)
    ...
    for k in range(opt["k_fold"]): # k-fold cross 
        ...
        train_index = []
        test_index = indexes[k*6000:(1+k)*6000]
